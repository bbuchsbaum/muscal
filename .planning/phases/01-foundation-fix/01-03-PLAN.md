...
<task type="auto">
  ...
  <files>tests/testthat/test-mfa.R</files>
  <action>
...
**Discovery note:** The files_modified lists test-mfa.R as the most likely location based on
this plan's focus on MFA functionality. If the failing test is in a different file, update
files_modified in this plan's frontmatter accordingly before making changes.
  </action>
  <verify>
...
<task type="auto">
  ...
  <files>R/mfa.R</files>
  <action>
**Important:** Plan 01 also modifies R/mfa.R (fixing chkor() calls). Since this plan runs in
Wave 2 after Plan 01 completes, start by reading the current state of R/mfa.R to see Plan 01's
changes. Do not revert those changes - work with the updated file.
