...
must_haves:
  ...
  artifacts:
    ...
    - path: "DESCRIPTION"
      provides: "Clean Imports without RcppEigen"
      not_contains: "RcppEigen"
    - path: "NAMESPACE"
...
<task type="auto">
  <name>Task 1: Remove RcppEigen dependency</name>
...
2. In DESCRIPTION:
   ...
   - Keep RcppEigen in LinkingTo for now (line 47) - it's auto-included by Rcpp::compileAttributes() in RcppExports.cpp and removing from LinkingTo would require regenerating that file
...
- `grep "RcppEigen" DESCRIPTION` only shows LinkingTo line
- `devtools::document()` regenerates NAMESPACE without import(RcppEigen)
  </verify>
  <done>RcppEigen removed from R imports, only retained in LinkingTo for C++ compilation</done>
</task>
...
Note: Some NOTEs are acceptable (e.g., "checking CRAN incoming feasibility" notes about new submission). The compiler pragma warning about '-Wfixed-enum-extension' is environment-specific (Homebrew clang) and not addressable at package level.
  ...
  <verify>
- R CMD check output shows "Status: OK" or "Status: X NOTE" (no errors, no warnings)
- No "replacing previous import" in check output
- No "undefined global functions" NOTE mentioning coef, shape, or multidesign
  </verify>
...
- NAMESPACE includes: importFrom(stats,coef), importFrom(multivarious,shape)
