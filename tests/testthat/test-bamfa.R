...
test_that("bamfa.list assigns default block names", {
  ...
  names(blocks) <- NULL
  res <- bamfa(blocks, k_g = 1L, k_l = 1L, niter = 1L, preproc = multivarious::pass())
...
test_that("bamfa.multiblock checks row consistency", {
  ...
  expect_error(bamfa(mb, k_g = 1L, k_l = 1L, niter = 1L, preproc = multivarious::pass()))
})
...
test_that("bamfa.multidesign requires subject argument", {
  ...
  expect_error(bamfa(md, k_g = 1L, k_l = 1L, niter = 1L, preproc = multivarious::pass()))
})
