...
``` r
# S3 method for class 'coupled_graph_anchored_mfa'
predict(
  object,
  new_data,
  block,
...
- object:
  ...
  A fitted \`coupled_graph_anchored_mfa\` object.
...
reconstructed block rows when \`type = "reconstruction"\`, or
block-specific latent scores when \`type = "scores"\`.
